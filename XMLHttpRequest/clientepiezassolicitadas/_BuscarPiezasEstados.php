...
/*	*/
    ...
    //CONSULTA CON FECHA OPTIMIZADA
        $Consulta = "
            ...
            FROM
                sispoc5_Ocasa.Piezas_Estados AS pe
            INNER JOIN(
                SELECT
                    pe.idPieza AS 'idPieza'
                FROM
            ...
            ON
                pe.idPieza = EstadoMinimo.idPieza AND pe.Fecha = EstadoMinimo.Fecha
            WHERE
                pe.BarcodeExterno NOT LIKE '' AND(
                    pe.Fecha >= '$Desde' AND pe.Fecha <= '$Hasta'
                ) AND pe.BarcodeExterno NOT LIKE ''
            GROUP BY
                pe.idPieza
            ) AS pef
            ON
                pef.idPieza = pe.idPieza
            INNER JOIN(
                SELECT
                    pe.idPieza AS 'idPieza',
                    pe.Fecha AS 'Fecha',
                ...
                INNER JOIN(
                    ...
                    ON
                        pe.idPieza = EstadoMinimo.idPieza AND EstadoMinimo.Fecha = pe.Fecha
                    GROUP BY
                        pe.idPieza
                    ) AS pef
                ON
            ...
            ON
                pef.idPieza = pe.idPieza AND pef.Fecha = pe.Fecha
            ) AS ue
            ON
                ue.idPieza = pe.idPieza
            INNER JOIN sispoc5_gestionpostal.flash_piezas AS fp
            ON
                fp.id = pe.idPieza
            INNER JOIN sispoc5_gestionpostal.flash_comprobantes_ingresos AS fci
            ON
                fci.id = fp.comprobante_ingreso_id
            INNER JOIN sispoc5_gestionpostal.flash_comprobantes_ingresos_servicios AS fcis
            ON
                fcis.id = fp.servicio_id
            INNER JOIN sispoc5_Ocasa.EstadoEntregaApp AS eeApp
            ON
                eeApp.id = pe.idEstados
            INNER JOIN sispoc5_Ocasa.EstadoEntregaApp AS ueeApp
            ON
                ueeApp.id = ue.idEstados
            LEFT JOIN sispoc5_gestionpostal.flash_sucursales AS fs
            ON
                fs.id = fp.sucursal_id
            LEFT JOIN sispoc5_Ocasa.VinculoDestinatario AS vd
            ON
                pe.idVinculo = vd.id
    ...
    //CONSULTA SIN FECHA CON PIEZA
        $Consulta = "
            ...
            INNER JOIN(
                    ...
                    INNER JOIN(
                        ...
                        WHERE 
                            pe.BarcodeExterno NOT LIKE '' 
                            AND (fci.cliente_id = '$UserId')
                            AND (pe.BarcodeExterno = '$NumeroDePieza')
                            AND ( (pe.Fecha >= '$Desde' AND pe.Fecha <= '$Hasta') OR ''='')
                        GROUP BY pe.idPieza
            ...
            INNER JOIN(
                ...
                ON pef.idPieza = pe.idPieza AND pef.Fecha = pe.Fecha
