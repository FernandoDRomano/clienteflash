<?php
	...
	*/
	$Columnas = array("Estado","Fecha De Estado"
	);//"idEstado",
	$Consulta= "
		SELECT 
        	CASE
        	...
        	,fev.nombre as 'Estado'
        	,fpt.create as 'Fecha De Estado'
        ...
        where fp.id = '$IdPieza'
        order by fp.id, fpt.create desc;
	";
	/*
	$Columnas = array("Comprobante De Ingreso","Pieza Id","Cliente","Fecha De Solicitud"
	,"Barcode","Recibio","Documento","Parentesco","Fecha De Estado","Estado","Ultimo Estado"
	);
	$Consulta= "
		SELECT 
		...
		, BarcodeExterno as 'Barcode'
		, case 
			when PE.idEstados = '8'
			then PE.ApellidoNombreRecibio
			else ''
			end as 'Recibio'
		, case 
			when PE.idEstados = '8'
		...
		#, VD.Nombre as 'Parentesco'
		, PE.Fecha as 'Fecha De Estado'
		, PE.Latitud as 'Latitud'
		, PE.Longitud as 'Longitud'
		, EEAPP.NombreAPP as 'Estado'
		, case PE.Activo 
			when 0 THEN 'No'
			WHEN 1 THEN 'Si'
			END as 'Ultimo Estado'
		...
		inner join sispoc5_Ocasa.VinculoDestinatario as VD on VD.id = PE.idVinculo
