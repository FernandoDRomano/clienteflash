<?php
	...
	if(count($_FILES["image_uploads"]["name"]) == 1 and $_FILES["image_uploads"]["name"][0] == ""){
		echo("Error:Imagen No Seleccionada:");
		exit;
	}
	for($i = 0 ; $i < count($_FILES["image_uploads"]["name"]); $i++){
		if($_FILES["image_uploads"]["error"][$i] == 0){
			$BaseFileName = makeRandomString(50) . ".". pathinfo( basename($_FILES["image_uploads"]["name"][$i]) , PATHINFO_EXTENSION);
	...
	$Columnas = array("id");
	$Consulta= "
		UPDATE sispoc5_correoflash.cliente 
		SET URLFirma = '$BaseFileName'
		WHERE Id = '$UserId'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,false);
	
	if($Resultado){
		echo("<P>Firma Agregada Correctamente</P>"."");	
	}else{
		echo("Ocurrio Un Errr Al Intentar Asignar La Firma");
	}
