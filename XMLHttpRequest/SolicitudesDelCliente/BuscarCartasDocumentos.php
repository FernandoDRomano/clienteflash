<?php
	...
	*/
	...
	$Columnas = array("Comprobante De Ingreso","Pieza Id","Cliente","Fecha De Creacion","RemitenteNombre","RemitenteApellido", "RemitenteCodigoPostal","RemitenteProvincia"
	...
	$Consulta= "
		...
		, FP.id as 'Pieza Id'
		, FP.create as 'Fecha De Creacion'
		, FCD.RemitenteNombre as 'RemitenteNombre'
		, FCD.RemitenteApellido as 'RemitenteApellido'
		, FCD.RemitenteCodigoPostal as 'RemitenteCodigoPostal'
		, CASE
			WHEN FCD.RemitenteProvincia REGEXP '^[0-9]+$' and FCD.RemitenteProvincia not like '0'
            	THEN concat((select NombreImprimible FROM sispoc5_correoflash.provincias WHERE id = FCD.RemitenteProvincia),'')
			ELSE 
				FCD.RemitenteProvincia
			END AS 'RemitenteProvincia'
		, CASE
			WHEN FCD.RemitenteLocalidad REGEXP '^[0-9]+$' and FCD.RemitenteLocalidad not like '0'
				THEN (SELECT NombreImprimible FROM sispoc5_correoflash.localidades where id = FCD.RemitenteLocalidad)
			ELSE 
				FCD.RemitenteLocalidad
			END AS 'RemitenteLocalidad'
		, FCD.RemitenteCalle as 'RemitenteCalle'
		, FCD.RemitenteNumero as 'RemitenteNumero'
		, FCD.RemitentePiso as 'RemitentePiso'
		, FCD.RemitenteDepartamento as 'RemitenteDepartamento'
		, FCD.DestinatarioNombre as 'DestinatarioNombre'
		, FCD.DestinatarioApellido as 'DestinatarioApellido'
		, FCD.DestinatarioCodigoPostal as 'DestinatarioCodigoPostal'
		, CASE
			...
			END AS 'DestinatarioProvincia'
		, CASE
			WHEN FCD.DestinatarioLocalidad REGEXP '^[0-9]+$' and FCD.DestinatarioLocalidad not like '0'
				THEN (SELECT NombreImprimible FROM sispoc5_correoflash.localidades where id = FCD.DestinatarioLocalidad)
			ELSE 
				FCD.DestinatarioLocalidad
			END AS 'DestinatarioLocalidad'
		, FCD.DestinatarioCalle as 'DestinatarioCalle'
		, FCD.DestinatarioNumero as 'DestinatarioNumero'
		, FCD.DestinatarioPiso as 'DestinatarioPiso'
		, FCD.DestinatarioDepartamento as 'DestinatarioDepartamento'
		, FCD.RemitenteEmail as 'RemitenteEmail'
		, FCD.RemitenteCelular as 'RemitenteCelular'
		, FCD.RemitenteObservaciones as 'RemitenteObservaciones'
		, FCD.ApoderadoNombre as 'ApoderadoNombre'
		, FCD.ApoderadoApellido as 'ApoderadoApellido'
		, FCD.ApoderadoDNITipo as 'ApoderadoDNITipo'
		, FCD.ApoderadoDocumento as 'ApoderadoDocumento'
		, FCD.ApoderadoFirma as 'ApoderadoFirma'
		, FCD.Formulario as 'Formulario'
		, FCD.URLPDF as 'URLPDF'
		...
		inner join sispoc5_gestionpostal.flash_piezas_cd as FCD on FP.id = FCD.IdFlashPieza
		where
		(
	...
	if($Resultado){
	    ...
	    echo 'la consulta no cumple con las uniones necesarias';
	}
?>
